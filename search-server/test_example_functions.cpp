...
// “ест провер€ет, что документы, содержащие минус-слова поискового запроса, не включютс€ в результаты поиска.
...
void TestFindAddedDocumentByWord() {
	...
	{	//»скомых слов в сервере не найдено
		const auto found_docs = server.FindTopDocuments("some words"s);
		ASSERT_EQUAL_HINT(found_docs.size(), size_t(0), "No some worsd in doc 777"s);
	}

	{	//«апрос содержит слова из добавленных документов
		const auto found_docs = server.FindTopDocuments("leave office"s);
		ASSERT_EQUAL(found_docs.size(), 1u);
		const Document& doc0 = found_docs[0];
	...
	{	//«апрос содержит слова из добавленных документов
		...
		const Document& doc0 = found_docs[0];
		ASSERT_EQUAL_HINT(doc0.id, doc1_id, "First founded doc is 13"s); //ѕервым вернулс€ 2й документ
		const Document& doc1 = found_docs[1];
		ASSERT_EQUAL_HINT(doc1.id, doc0_id, "First founded doc is 13"s); //¬торым вернулс€ первый документ
	}
}
// “ест провер€ет матчинг слов
...
// “ест провер€ет сортировку по релевантности
...
// “ест провер€ет вычисление рейтинга документов
